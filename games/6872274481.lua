...
local function getSpeed()
	...
	if lplr.Character then
		...
		if store.scythe > tick() then
			speed += (isEnabled('Desync') and render.clone.new and 53 or 18)
		end
					...
					repeat
						...
						if #plrs > 0 then
							...
							killauraboxes:clear(function(box: Instance)
								local fadeout: Tween = tween:Create(box, TweenInfo.new(0.2), {Transparency = 1});
								fadeout:Play();
...
run(function()
	...
	scythedisabler = utility.Api.CreateOptionsButton({
		Name = 'ScytheDisabler',
		HoverText = 'bypasses anti cheat with scythe.',
		Function = function(calling: boolean)
			if calling then 
				repeat 
					local scythe: table | nil = getItemNear('scythe');
					local oldhand: table | nil = getserverhand()
					if scythe and bedwars.CombatController and isAlive(lplr, true) then 
						switchItem(scythe.tool);
						bedwars.Client:Get('ScytheDash'):SendToServer({
							direction = (lplr.Character.PrimaryPart.CFrame.LookVector * 9e9)
						});
						if lplr:GetAttribute('ScytheSpinning') then 
							store.scythe = tick() + 1.3;
							if oldhand then 
		...
		Function = function(calling: boolean)
			if calling then 
				table.insert(autorewind.Connections, runservice.Heartbeat:Connect(function()
					local block: Raycast = isAlive(lplr, true) and workspace:Raycast(lplr.Character.PrimaryPart.Position, Vector3.new(0, -30, 0), store.blockRacyast);
					if block and not istweening then 
...
run(function()
	...
	local getscytheshopitem = function()
		if cheatenginetrash then
			return `2_item_shop_{math.random(1, 4)}`
		end;
		...
		Function = function(calling: boolean): ()
			if calling then 
				spawnscythethread = task.spawn(function()
					repeat
						local scythe: invitemobject? = getItemNear('_scythe');
						if isAlive(lplr, true) and not scythe and store.queueType then 
							local npc: Part? = getshopnpc();
							bedwars.Client:Get('BedwarsPurchaseItem'):CallServer({
								shopItem = getscytheshopitem(),
