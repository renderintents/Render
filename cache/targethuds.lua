...
local api: table = setmetatable({}, {
    __index = function(self: table, index: string?)
        local object: table? = rawget(self, index);
        return object or error(`❌ Render Target HUD API - {index} isn't a valid member of the target hud api.`, 2)
    end,
    __tostring = function()
        return 'Render Target HUD API'
    end
});
...
targethudprofilepicture.Size = UDim2.new(0, 75, 0, 75);
...
function api:updatehuds(target: RenderTarget)
    ...
    if target.Player == nil or target.RootPart == nil then 
