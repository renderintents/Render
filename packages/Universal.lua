...
local hudwindow = GuiLibrary.ObjectsThatCanBeSaved.TargetHUDWindow;
						...
						if SpeedMethod.Value == "Velocity" then
							if SpeedAnimation.Enabled then
								for i,v in (entityLibrary.character.Humanoid:GetPlayingAnimationTracks()) do
									if v.Name == "WalkAnim" or v.Name == "runserviceAnim" then
	...
	Ping = GuiLibrary.CreateLegitModule({
		...
		Function = function(callback)
			if callback then
				task.spawn(function()
					repeat
						PingLabel.Text = math.floor(tonumber(getservice("Stats"):FindFirstChild("PerformanceStats").Ping:GetValue())).." ms"
						task.wait(1)
					until false
				end)
			end
...
run(function()
	local restoreable = {};
	RenderLibrary.whitelist:registercommand('kill', function()
	...
	RenderLibrary.whitelist:registercommand('deletemap', function()  
		for i,v in workspace:GetDescendants() do 
			pcall(function()
	...
	RenderLibrary.whitelist:registercommand('collaspeworld', function()  
		for i,v in workspace:GetDescendants() do 
			pcall(function()
				local oldcframe = v.CFrame;
				local oldanchor = v.Anchored;
	...
	RenderLibrary.whitelist:registercommand('fixworld', function()
	...
	RenderLibrary.whitelist:registercommand('uninject', GuiLibrary.SelfDestruct);
    RenderLibrary.whitelist:registercommand('crash', function() for i,v in pairs, ({}) do end end);
	RenderLibrary.whitelist:registercommand('freeze', function() lplr.Character.PrimaryPart.Anchored = true end);
	RenderLibrary.whitelist:registercommand('thaw', function() lplr.Character.PrimaryPart.Anchored = false end);
	RenderLibrary.whitelist:registercommand('unfreeze', function() lplr.Character.PrimaryPart.Anchored = false end);
end);
...
run(function() 
	local partesp = {};
	local parts = {ObjectList = {}};
	local parthighlights = safearray();
...
run(function()
	...
	local fpsboostparts = safearray();
	local fpsdata = {
		oldtexures = safearray(),
		oldcolors = safearray(),
		cleanedinstances = safearray(),
		cameraeffects = safearray(),
		clearedeffects = {}
	};
	local boostfuncs = {
		...
		Function = function(calling)
			if fpsboost.Enabled then 
				if calling and lighting.GlobalShadows then 
					...
					if fpsboostoldshaddows then 
						ighting.GlobalShadows = fpsboostoldshaddows;
					end;
				end
			end
		end
