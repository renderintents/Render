if shared.VapeExecuted then
	...
	local success = pcall(function()
		return cloneref(game:FindService('CoreGui')).TopBarApp.TopBarFrame.LeftFrame:FindFirstChildOfClass('UIListLayout').Padding
	end)
	...
	if not newui then -- else would be a bit messy yk
		...
		rendericon.BackgroundTransparency = 1
		rendericon.Position = UDim2.new(0.094, 0, 0.094, 0)
		rendericon.Size = UDim2.new(0, 25, 0, 25)
		rendericon.Image = 'rbxassetid://16902612758'
		task.spawn(function()
			buttonmain.Parent = cloneref(game:FindService('CoreGui')):WaitForChild('TopBarApp'):WaitForChild('TopBarFrame'):WaitForChild('LeftFrame');
		end)
		button = buttonbkg
	end
	...
	task.spawn(function()
		...
		getgenv().render.button = {instance = button, isNewUI = newui};
	end)
