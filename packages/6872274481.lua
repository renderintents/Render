...
do
						...
						table.insert(newent.Connections, hum.AnimationPlayed:Connect(function(state)
							...
							if animnum then
								if not entityLibrary.animationCache[state.Animation.AnimationId] then
									entityLibrary.animationCache[state.Animation.AnimationId] = getservice('MarketplaceService'):GetProductInfo(animnum)
								end
	...
	textlabel.Parent = vape.MainGui.ScaledGui.ClickGui
end
		...
		Function = function(callback)
			...
			for i = 1, 10 do
				...
				if callback then
					...
					killaurabox.Parent = vape.MainGui
				else
			...
			if callback then
				...
				task.spawn(function()
					repeat 
						...
						--pcall(function() particle.Texture = killauraparticleicon.Value ~= '' and `rbxassetid://{killauraparticleicon.Value}` or oldtexture end);
				...
				task.spawn(function()
					repeat
						...
						if entityLibrary.isAlive then
							...
							local towering = ScaffoldTower.Enabled and inputservice:IsKeyDown(Enum.KeyCode.Space) and getservice('Userinputservice'):GetFocusedTextBox() == nil
							if towering then
...
if cheatenginetrash then -- aids code for cheat engine garbage such as solara.
	local damagefunc = function(character)
		if not character then 
			return;
		end;
		repeat task.wait() until character:GetAttribute('Health');
		local oldhp = character:GetAttribute('Health'); 
		table.insert(vapeConnections, character:GetAttributeChangedSignal('Health'):Connect(function()
			if character:GetAttribute('Health') < oldhp then 
				vapeEvents.EntityDamageEvent:Fire({entityInstance = character});
			end;
			oldhp = character:GetAttribute('Health');
		end));
	end;
	for i,v in players:GetPlayers() do 
		task.spawn(damagefunc, v.Character);
		table.insert(vapeConnections, v.CharacterAdded:Connect(damagefunc));
	end;
...
task.spawn(function()
	...
	if render.button.isNewUI == false then 
		...
		render.button.Visible = platform ~= Enum.Platform.Windows and platform ~= Enum.Platform.OSX;
	end
end);
...
task.spawn(function()
	repeat task.wait() until shared.VapeFullyLoaded;
	if rendervapequeued == nil and cheatenginetrash then 
		task.wait(0.3);
		warningNotification('Render', 'You\'re currently using a cheat engine executor. Some features may not work as intended/at all.', 15)
	end;
end)
...
run(function() -- pasted from old render once again
	...
	local hotbarcoloricons = safearray();
	local hotbarsloticons = safearray();
	local hotbarobjects = safearray();
	local HotbarVisualsGradient = {};
	local hotbarslotgradients = safearray();
...
run(function()
	...
	local detectedplayers = safearray();
	...
	local projaurasortfuncs = {
		Distance = function()
			local target = GetTarget({
				radius = projaurarangeoption.Enabled and projaurarange.Value or 2000, 
				npc = projauranpc.Enabled
			});
...
run(function()
	...
	local staffdetectorLoop = function(player: Player)
		stafftasks[#stafftasks + 1] = task.spawn(function() --> personally looks better
			local tags = player:WaitForChild('Tags', math.huge);
			local newtagconnection
			for i,v in tags:GetChildren() do 
			...
			newtagconnection = tags.ChildAdded:Connect(function(v)
				local match = matchtag(v);
				if match then
					errorNotification('StaffDetector', `A special player has been detected in your match (@{player.DisplayName} [{v.Value:upper()}]).`, 60);
					savestaffdata(player, 'TAG');
	...
	staffdetector = utility.Api.CreateOptionsButton({
		...
		Function = function(calling)
			if calling then 
				...
				for i,v in players:GetPlayers() do 
					stafftasks[#stafftasks + 1] = task.spawn(staffdetectorLoop, v)
				end;
				table.insert(staffdetector.Connections, players.PlayerAdded:Connect(function(v)
					stafftasks[#stafftasks + 1] = task.spawn(staffdetectorLoop, v)
				end))
			else 
				for i,v in stafftasks do 
...
run(function()
	...
	mousetp = world.Api.CreateOptionsButton({
		...
		Function = function(calling: boolean)
			if calling then 
				local block = workspace:Raycast(camera.CFrame.Position, lplr:GetMouse().UnitRay.Direction * 1e4, store.blockRaycast);
				if block then 
