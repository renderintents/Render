...
run(function()
	...
	local targetstraferange1 = {Value = 14}
	local targetstraferange2 = {Value = 5}
	local oldmove
	local controlmodule
	targetstrafe = rendervape.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				...
				controlmodule.moveFunction = function(Self, vec, facecam, ...)
					if entityLibrary.isAlive then
						local plr = EntityNearPosition(targetstraferange1.Value, {
							WallCheck = false,
							AimPart = "RootPart"
						})
						if plr then
							...
							local c, s
							--print(typeof(plr.RootPart.Position.X))
							local plrCFrame = CFrame.lookAt(entityLibrary.character.HumanoidRootPart.Position, Vector3.new(plr.RootPart.Position.X + (targetstraferange2.Value + 3), 0, plr.RootPart.Position.Z))
	...
	targetstraferange1 = targetstrafe.CreateSlider({
		Name = "Range",
		Function = void,
		Min = 1,
		Max = 100,
	...
	targetstraferange2 = targetstrafe.CreateSlider({
		Name = 'Range2',
		Function = void,
		Min = 0,
		Max = 20,
                    ...
                    repeat
                        ...
                        if isAlive(lplr, true) then
                            ...
                            if lastTeleportSeconds >= (0.1 * desyncdelay.Value) then 
                                lastTeleport = tick();
                                oldroot.CFrame = (newroot.CFrame + Vector3.new(0, desyncupvelo.Value, 0))
                            end
                        end
	...
	Atmosphere = rendervape.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then 
				...
				rendervape.ObjectsThatCanBeSaved.LightingModsOptionsButton.Api:retoggle();
			else
				if skyobj then skyobj:Destroy() end
				if skyatmosphereobj then skyatmosphereobj:Destroy() end
				for i,v in (oldobjects) do 
...
loadfile('rendervape/libraries/rain.lua')()
...
run(function()
	...
	local createtrailpart = function()
		...
		task.delay(2.5, function()
			tween:Create(part, TweenInfo.new(0.6, Enum.EasingStyle.Quad), {Size = Vector3.zero}):Play()
			task.wait(0.6);
			part:Destroy();
		end);
		return part
	...
	guifonts = visual.Api.CreateOptionsButton({
		...
		Function = function(calling)
			if calling then
				...
				for i,v in next, rendervape.ObjectsThatCanBeSaved do 
					if v.Type == 'OptionsButton' then 
...
run(function()
	...
	local createfpsboostTab: () -> safetable = function()
		local cleaner: safetable = Performance.new(setmetatable({
			jobdelay = 5, 
		}, {
			__index = function(self: table, index: string)
	...
	PlayerViewModel = rendervape.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
		...
		Function = function(call)
			if call then
				...
				RunLoops:BindToHeartbeat('PlayerVM', false, function()
					for i,v in players:GetPlayers() do
