    ...
    local api = {events = {}, uninjected = Instance.new('BindableEvent')}
    ...
    function api:makedrag(ui: Instance, speed: number, uninjectbypass: boolean, openflag: boolean)
        ...
        local dragfunc = function(input)
            ...
            cloneref(game:FindService('TweenService')):Create(ui, TweenInfo.new(tonumber(speed or '') or 0.05), {Position = UDim2.new(startpos.X.Scale, startpos.X.Offset + delta.X, startpos.Y.Scale, startpos.Y.Offset + delta.Y)}):Play()
        end
    ...
    function api:switchtab(tab: table, animation: boolean, noclear: boolean)
        ...
        for i,v in next, api.window.tabs do
            if v.Selected and tostring(v) ~= tab then 
                ...
                local out = cloneref(game:FindService('TweenService')):Create(v.Instance.Frame, TweenInfo.new(animation and 0.21 or 0.001, Enum.EasingStyle.Quad), {Size = UDim2.new(0, 0, 0, -2)})
                out:Play()
                task.spawn(function()
                    out.Completed:Wait()
            ...
            if tostring(v) == tab and not v.Selected then
                ...
                cloneref(game:FindService('TweenService')):Create(v.Instance.Frame, TweenInfo.new(animation and 0.2 or 0.001, Enum.EasingStyle.Quad), {Size = UDim2.new(0, 60, 0, -2)}):Play()
                switched = true
                if not noclear then 
                    api:cleartab()
                end
        ...
        function api:install(steptab: table)
            ...
            for i,v in next, api.window.Instance.Frame:GetDescendants() do 
                if v ~= api.window.definitions.top and v ~= api.window.definitions.topicon and (v.ClassName:lower():find('label') or v.ClassName:lower():find('button') or v.ClassName == 'Frame') then 
                    task.spawn(function()
                        local invis = cloneref(game:FindService('TweenService')):Create(v, TweenInfo.new(0.7, Enum.EasingStyle.Quad), {BackgroundTransparency = 1})
                        pcall(function() cloneref(game:FindService('TweenService')):Create(v, TweenInfo.new(0.7, Enum.EasingStyle.Quad), {ImageColor3 = 1}):Play() end)
                        pcall(function() cloneref(game:FindService('TweenService')):Create(v, TweenInfo.new(0.7, Enum.EasingStyle.Quad), {TextTransparency = 1}):Play() end)
                        pcall(function() cloneref(game:FindService('TweenService')):Create(v, TweenInfo.new(0.7, Enum.EasingStyle.Quad), {ImageTransparency = 1}):Play() end)
                        pcall(function() cloneref(game:FindService('TweenService')):Create(v, TweenInfo.new(0.7, Enum.EasingStyle.Quad), {Transparency = 1}):Play() end)
                        invis:Play()
            ...
            cloneref(game:FindService('TweenService')):Create(api.window.Instance.Frame, TweenInfo.new(0.5, Enum.EasingStyle.Sine), {Size = UDim2.new(0, 449, 0, 211)}):Play()
            cloneref(game:FindService('TweenService')):Create(api.window.definitions.topicon, TweenInfo.new(0.5, Enum.EasingStyle.Sine), {Position = UDim2.new(0.082, 0, 0.082, 0)}):Play()
            task.wait(0.5)
            ...
            function installapi:start()
                for i, func in next, steps do 
                    ...
                    cloneref(game:FindService('TweenService')):Create(installerbar2, TweenInfo.new(0.5, Enum.EasingStyle.Quad), {Size = UDim2.new(0, (#steps - i) <= 0 and 364 or 364 / (#steps - i), 0, 16)}):Play()
                    if not success then 
    ...
    function api:createbutton(userdata: table)
        ...
        function buttonapi:toggle(mode: boolean)
            ...
            if buttonapi.enabled then 
                cloneref(game:FindService('TweenService')):Create(button, TweenInfo.new(0.1, Enum.EasingStyle.Quad), {BackgroundColor3 = Color.fromRGB(10, 0, 43)}):Play()
                cloneref(game:FindService('TweenService')):Create(buttontext, TweenInfo.new(0.1, Enum.EasingStyle.Quad), {TextColor3 = Color.fromRGB(255, 255, 255)}):Play()
                cloneref(game:FindService('TweenService')):Create(buttonicon, TweenInfo.new(0.1, Enum.EasingStyle.Quad), {ImageColor3 = Color.new(255, 255, 255)}):Play()
            else 
                cloneref(game:FindService('TweenService')):Create(button, TweenInfo.new(0.1, Enum.EasingStyle.Quad), {BackgroundColor3 = Color.fromRGB(255, 255, 255)}):Play()
                cloneref(game:FindService('TweenService')):Create(buttonicon, TweenInfo.new(0.1, Enum.EasingStyle.Quad), {ImageColor3 = Color.new()}):Play()
                cloneref(game:FindService('TweenService')):Create(buttontext, TweenInfo.new(0.1, Enum.EasingStyle.Quad), {TextColor3 = Color.new()}):Play()
            end
